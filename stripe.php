	...
	$link = mysql_connect('localhost', 'root','m2n1shlko@1250');

	if (!$link) {
		die('<br>Could not connect: ' . mysql_error());
	}
	$db_selected = mysql_select_db('licensemanager', $link);

	/* Database connection code ends here */

	\Stripe\Stripe::setApiKey("sk_test_m8P8ZI2U04WvoSxYdF72mSMx");
	...
	*/
	...
	if (mysql_num_rows($sql_res_check)===0){
		...
		if($sql_res_2 === TRUE) {
			...
			$sql_license = "INSERT INTO `licenses` (`licensekey`, `ip_count`,`customer_email`, `expirydate`, `productid`, `product_purchase_stripe_id`, `status`, `issued-by`, `comments`, `parameters`) VALUES ('$LicenseKey', '1', '$customer_email', '$expirydate', '$productid','$purchase_plan_id', '$status', '1', '$comments', '$parameters');";      
